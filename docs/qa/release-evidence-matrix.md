...
| Developer Workstation | passed | `cargo install --git https://github.com/solana-foundation/anchor avm --force`, `avm install latest`, `anchor --version`, and `scripts/workstation-localnet-smoke.sh --full-localnet` | local/live | `docs/qa/developer-workstation-localnet-smoke.md` | Anchor CLI `1.0.2` is active and Rust/Cargo `1.95.0` are active; `avm use latest` still panics locally, but full localnet sample deploy succeeded | Keep AVM runtime panic documented; continue to block mainnet program ops and unverified binary installs |
