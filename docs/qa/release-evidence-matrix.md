...
| Developer Workstation | blocked | `scripts/workstation-localnet-smoke.sh --check` and `scripts/workstation-localnet-smoke.sh --full-localnet` | local/live-blocked | `docs/qa/developer-workstation-localnet-smoke.md` | Anchor activation failed; `anchor --version` reports no active Anchor version after `avm install 0.30.1` failed compiling `time 0.3.29` under Rust/Cargo 1.94 | Provide a verified compatible Anchor/AVM artifact or compatible Rust toolchain, then rerun full localnet smoke |
