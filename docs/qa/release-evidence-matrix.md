...
| Developer Workstation | blocked | `rustup toolchain install stable`, `avm install latest`, `anchor --version`, `scripts/workstation-localnet-smoke.sh --check`, and `scripts/workstation-localnet-smoke.sh --full-localnet` | local/live-blocked | `docs/qa/developer-workstation-localnet-smoke.md` | Rust stable activated to `1.95.0`, but Anchor `latest` resolved to `1.0.2` and failed during native linking; Anchor remains inactive and `anchor --version` still reports inactive | Resolve the Anchor 1.0.2 linker blocker or pin an official verified prebuilt artifact, then rerun full localnet smoke |
