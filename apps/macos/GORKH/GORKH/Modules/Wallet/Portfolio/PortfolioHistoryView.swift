                    ...
                    ForEach(snapshots.prefix(8)) { snapshot in
                        HStack {
                            VStack(alignment: .leading, spacing: 3) {
                                ...
                                Text("\(snapshot.walletCount) wallets / \(snapshot.assetCount) assets / \(snapshot.stakeAccountCount) stake / \(snapshot.lstHoldingCount) LST / \(snapshot.lendingPositionCount) lending / \(snapshot.unavailablePriceCount) missing prices")
                                    .font(.caption)
                                    .foregroundStyle(GorkhColors.secondaryText)
                            }
                            Spacer()
